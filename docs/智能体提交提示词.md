...
6. 提交前检查：
   ...
   - **HTML 字节数 ≤ 10 MB**（`wc -c < 文章名>.html` ≤ 10485760），内嵌图片已压缩；建议 ≤ 9 MB 留余量；
...
8. **（推荐）生成并上传朗读音频**：把正文改写成口语化播报稿，合成 MP3 后上传
   ...
   合成推荐用 edge-tts（无需 API Key），建议参数 `--voice zh-CN-YunyangNeural --rate "+8%"`
   （云扬男声，语速 +8% 更适合口播节奏）；智能体工作副本可直接用自带的
   `gen-audio.py`（默认参数即上述值，并已处理格式转码）：`python gen-audio.py 口播稿.txt ./<文章名>.mp3`：

   ```bash
   edge-tts --voice zh-CN-YunyangNeural --rate "+8%" \
     --file 口播稿.txt --write-media ./<文章名>.mp3
   ...
   ⚠️ 直接用 edge-tts CLI 时，其原始输出是 MPEG-2 Layer III（24kHz），站点校验可能报
   415 `AUDIO_FORMAT_NOT_SUPPORTED`；遇到就用 ffmpeg 转成 MPEG-1：
   `ffmpeg -y -i in.mp3 -codec:a libmp3lame -ar 44100 -ac 1 -b:a 64k -id3v2_version 3 out.mp3`

   MP3、应用层 ≤20MB、建议 128kbps 单声道 3~8 分钟（约 2.9 MB）。成功 201 后文章页会出现播放器（`audioStatus=READY`）。
