...
1. 在项目根目录准备好不会提交到 Git 的 `.env.agent`。
2. 把下面提示词中的 `{{UPLOADER_ID}}` 和 `{{DEFAULT_CATEGORY}}` 替换为该智能体的固定值。

不要把共享上传密码写进提示词。推送命令会自动从 `.env.agent` 读取。
...
如果 `.env.agent` 不存在、认证失败或栏目无效，请停止提交并通知网站所有者处理。不要自行猜测密码。
