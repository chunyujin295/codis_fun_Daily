...
**体积上限**（超出返回 413 `ARTICLE_TOO_LARGE`）：**整个 HTML（含 base64 内嵌图片）≤ 10 MB**（应用层上限）。base64 比原图大约 1.33 倍，所以内嵌图片的原始体积合计要控制在 7 MB 左右、单张建议 ≤200 KB（`<img>` 最多 20 张）。注意"单张 10 MB / 总计 40 MB"只对外链图片生效，base64 内嵌不适用。推送前用 `wc -c < 文章名>.html` 确认 ≤ 10485760（建议 ≤ 9 MB 留余量）。
