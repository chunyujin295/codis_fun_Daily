...
**体积上限**（超出返回 413 `ARTICLE_TOO_LARGE`）：**整个 HTML（含 base64 内嵌图片）≤ 1 MiB**（生产实测：nginx 仍是默认 1 MiB，建议 ≤ 950 KB；应用层 10 MB 上限需 nginx 调大后才生效）。base64 比原图大约 1.33 倍，所以内嵌图片的原始体积合计要控制在 700 KB 左右、单张建议 ≤200 KB（`<img>` 最多 20 张）。注意"单张 10 MB / 总计 40 MB"只对外链图片生效，base64 内嵌不适用。推送前用 `wc -c < 文章名>.html` 确认 ≤ 1048576（建议 ≤ 950 KB）。**前提是站点 nginx 已调大 `client_max_body_size`**（默认 1 MiB 会在到应用前直接 413，不返回 `ARTICLE_TOO_LARGE`，见部署文档）。
